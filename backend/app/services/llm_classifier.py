...
class AnnouncementExtraction(BaseModel):
    ...
    project_category: Optional[str] = Field(default="", description="业务类别(机会列表列): 广告创意设计/物料制作印刷/活动策划执行/品牌宣传传播/视频内容制作/新媒体运营/媒介资源投放/渠道营销推广/通信工程建设/设计勘察/ICT系统集成/软件开发/大数据AI/信息安全/设备采购/网络维护代维/行政物业/食堂餐饮等")
...
任务2 — 从公告标题和正文中提取以下字段（无论是否广告营销类，只要正文中存在就提取；不存在则返回 null）。
这些字段对应系统"机会列表"表格的列，请严格按字段名输出：

- "is_ad": true/false —— 是否广告营销类
- "category": "赛道名" —— 广告赛道分类；非广告类为 "" 或 null
- "reason": "判断理由" —— 30字以内
- "project_category": "业务类别" —— 机会列表"业务类别"列。
  只能从以下类别中选一个（不得自创、不得用斜杠组合多个类别）：
  广告创意设计/物料制作印刷/活动策划执行/品牌宣传传播/视频内容制作/新媒体运营/
  媒介资源投放/渠道营销推广/通信工程建设/设计勘察/ICT系统集成/软件开发/大数据AI/信息安全/
  设备采购/服务器存储/网络设备/终端及配套/设备维保/网络维护代维/行政物业/食堂餐饮/办公后勤
  判断标准：看采购内容核心是什么（制作→物料制作印刷；投放→媒介资源投放；
  设计→广告创意设计；活动→活动策划执行；宣传/品牌→品牌宣传传播；软件开发→软件开发；
  系统集成→ICT系统集成；云/大数据/AI→大数据AI；安全→信息安全；服务器/存储→服务器存储；
  网络设备→网络设备；维保/维护→设备维保或网络维护代维；其他明确类别选最接近的）
  实在无法归入以上任何类别才用"其他采购"
- "budget": 数字或null —— 预算金额（万元，原文以"元"为单位请除以10000；有多个标段取合计）
- "registration_fee": 数字或null —— 报名费/标书费（元）
- "deposit": 数字或null —— 投标保证金（万元）
- "deadline": "YYYY-MM-DD"或null —— 机会列表"报名/反馈截止日期"列。
  定义：报名截止/反馈截止/招募文件获取截止。对应正文表述：
  1. "报名截止时间/日期：X"
  2. "招募文件获取时间：X至Y" → 取区间结束日 Y（报名截止 = 获取截止）
  3. "购买/售卖标书截止时间：X"
  注意：招募公告中"招募文件获取时间：X至Y"必须取 Y（结束日），绝不能取 X（开始日）；
  应答截止时间/递交截止时间不是报名截止，不要映射到 deadline
- "bid_date": "YYYY-MM-DD"或null —— 机会列表"投标日期"列。
  定义：投标/开标/应答截止日期。对应正文表述（按优先级）：
...
- "procurement_method": "方式名"或null —— 采购方式（公开招标/公开询比/竞争性谈判/单一来源/邀请招标/询价/比选/公开招募）
