(function() {
    ...
    2: [ function(D, h, z) {
        ...
        const l = "https://translate.google.com/translate_a/element.js?cb=googleTranslateElementInit&hl=%LANG%", Z = document.createElement("img");
        Z.id = "translator-icon", Z.src = chrome.runtime.getURL("/icons/icon-128.png"),
        document.body.appendChild(Z);
        const A = document.createElement("div");
        A.id = "translator-container";
        const q = chrome.runtime.getURL("icons/logo.svg"), Q = chrome.runtime.getURL("icons/copy.svg"), I = chrome.runtime.getURL("icons/r-arrow.svg");
        A.insertAdjacentHTML("beforeend", `\n<div class="rapid-header">\n  <img src="${q}">\n  <img id="rapid-copy" src="${Q}">\n</div>`);
        const E = document.createElement("div");
        E.id = "rapid-text-wrapper";
        const X = document.createElement("span");
        E.appendChild(X), A.appendChild(E), A.insertAdjacentHTML("beforeend", `\n\n<div class="rapid-footer">\n  <span id="rapid-first-language">ENG</span>\n  <img id="rapid-arrow-icon" src="${I}">\n  <span id="rapid-second-language">FRN</span>\n</div>`),
        document.body.appendChild(A);
        let f = null, s = false;
        (0, j.default)("#rapid-copy").click((() => {
            const D = document.createElement("textarea");
            D.value = X.textContent, document.body.appendChild(D), D.select(), document.execCommand("Copy"),
            D.remove();
        })), setInterval((() => {
            if (f) return;
            A.style.display = "none", Z.style.display = "none";
        }), 250), Z.addEventListener("click", (() => {
            if (s || !f) return;
            s = true;
            let D = "en";
            chrome.storage.sync.get([ "language" ], (h => {
                if (h.language) D = h.language;
                A.style.display = "none", X.innerHTML = "", j.default.ajax({
                    ...
                    success: h => {
                        const z = (0, j.default)(window).width(), F = (0, j.default)(window).height();
                        if ((0, j.default)("#rapid-first-language").html(h.src.toUpperCase()), (0, j.default)("#rapid-second-language").html(D.toUpperCase()),
                        s = false, h.sentences && h.sentences.length) for (const D of h.sentences) if (D.trans) X.innerHTML += D.trans;
                        const l = Z.getBoundingClientRect().x, q = Z.getBoundingClientRect().y;
                        if (A.style.display = "flex", l > z / 2) A.style.left = `${Z.offsetLeft - A.offsetWidth}px`; else A.style.left = `${Z.offsetLeft}px`;
                        if (q > F / 2) A.style.top = `${Z.offsetTop - A.offsetHeight}px`; else A.style.top = `${Z.offsetTop}px`;
                    },
                    error: () => {
                        s = false;
                    }
            ...
            if (D.target.closest("#translator-container")) return;
            f = null;
            const h = window.getSelection();
            if (!h) return;
            if (h.type !== "Range" || h.toString().length === 0) return;
            try {
                const D = h.getRangeAt(0), z = D.getBoundingClientRect();
                A.style.display = "none", Z.style.display = "none", f = h;
                const j = document.createRange();
                j.selectNode(f.focusNode);
                const F = j.getBoundingClientRect();
                Z.style.top = `${window.scrollY + F.y + F.height + 5}px`, Z.style.left = `${z.x + z.width / 2}px`,
                Z.style.display = "flex";
            } catch (D) {}
        })), window.addEventListener("message", (async D => {
            if ((D === null || D === void 0 ? void 0 : D.data) === "getsettings") {
                const {pageEnabled: D, translateUrl: h, abbv: z} = await chrome.storage.sync.get([ "pageEnabled", "translateUrl", "abbv" ]);
                let j = (h || l).replaceAll("%LANG%", z);
                const F = !D && !h;
                if (!F) window.postMessage({
                    pageEnabled: D,
                    translateUrl: j,
                    abbv: z
                }, "*");
